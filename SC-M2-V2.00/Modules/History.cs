...
namespace SC_M2_V2._00.Modules
{
    public class History
    {
        ...
        public void Save()
        {
            string sql = "insert into history (name,name_sw,name_lb,result,created_at,updated_at) values (@name,@name_sw,@name_lb,@result,@created_at,@updated_at)";
        ...
        public void Update()
        {
            string sql = "update history set name = @name, name_sw = @name_sw, name_lb = @name_lb, result = @result, updated_at = @updated_at where id = @id";
            Dictionary<string, object> parameters = new Dictionary<string, object>();
